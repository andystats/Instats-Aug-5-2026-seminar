...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=west, text=accent, font=\fontsize{14.5}{18}\selectfont]
      ...
      {Data Can Deceive \(\cdot\) Causal Roadmaps with (and without) AI};
...
\begin{frame}{Overview: The Causal Roadmap}
  ...
  \begin{itemize}
    \item The 9-step Causal Roadmap framework
    \item A worked example: pneumococcal vaccine and pneumonia
    \item How Target Trial Emulation structures observational studies
    \item Introduction to the interactive Causal Roadmap tool (Causal Navigator)
  \end{itemize}
  \vspace{0.4em}
  ...
  (7--9 depending on grouping); the Causal Navigator uses eight.
  Same workflow --- we use 9 for clarity.\par}
\end{frame}

\section{Why a Roadmap?}

\begin{frame}{Especially important to surface up assumptions}

\begin{figure}
	\centering
	\includegraphics[width=0.55\linewidth]{art/iceberg}
\end{figure}
...
\begin{frame}{Motivating Example: Pneumococcal Vaccine}
  ...
  \textbf{Real-world facts:}
  \begin{itemize}
    \item Older adults are more likely to be vaccinated
    \item Older adults are more likely to get pneumonia
  \end{itemize}
  \vspace{0.25em}
  ...
  \textcolor{red}{\textbf{Na\"ive conclusion:}} The vaccine \emph{increases} pneumonia!
  \vspace{0.25em}
...
\begin{frame}{The Causal Roadmap: A 9-Step Framework}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.66\textwidth}
      \centering
      \includegraphics[height=0.85\textheight]{art/roadmap9.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Step 1 Applied: Pneumonia Vaccine}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item Adults aged 65+ in health system Y
      \end{itemize}
      \vspace{0.3em}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \textbf{Context:}
      \begin{itemize}
        \item Real-world EHR data
      \end{itemize}
      \vspace{0.3em}

      \textbf{Decision context:}
      \begin{itemize}
        \item Should clinicians recommend vaccination to older adults?
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Step 2 Applied: The Correct DAG (Directed Acyclic Graph)}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=1.0]
          \node[draw, circle, fill=accent!20] (L) at (1.5,1.5) {$L$};
          \node[draw, circle, fill=green!20] (A) at (0,0) {$A$};
          \node[draw, circle, fill=orange!20] (Y) at (3,0) {$Y$};
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{itemize}
        \item Older/sicker adults more likely to be vaccinated
        \item Same factors increase pneumonia risk
        \item $L$ confounds the $A \to Y$ relationship
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
% ----------------------------------------
\begin{frame}{Step 3: Define the Causal Parameter}
  \small
  ...
  \textbf{Other common causal parameters:}
  \begin{itemize}
  ...
  \textcolor{accent}{\textbf{For our example:}} ATE = causal reduction in 1-year pneumonia risk due to vaccination, in adults 65+.
\end{frame}
...
\begin{frame}{Step 4: Describe the Observed Data}
  ...
  \textbf{For the vaccine example (EHR data):}
  \begin{itemize}
    \item $L$: Age and comorbidities (baseline confounders) --- reliably captured
    \item $A$: Vaccination status --- structured EHR fields
    \item $Y$: Pneumonia hospitalization --- ICD-coded
  \end{itemize}
  \vspace{0.2em}

  \textbf{Data structure:} \quad $O = (L, A, Y)$ for each patient
  \vspace{0.2em}

  \textcolor{accent}{\textbf{Key question:}} Do we have all confounders in $L$? This determines identifiability.
\end{frame}
...
\begin{frame}{Step 5: Assess Identifiability}
  ...
  \begin{itemize}
    ...
    \item \textbf{Exchangeability:} $Y^{(a)} \perp\!\!\!\perp A \mid L$ (no unmeasured confounding)
    \item \textbf{Positivity:} $P(A=a \mid L) > 0$ for all $L$ and $a$
  \end{itemize}
  \vspace{0.2em}

  \textbf{For the vaccine example:} if $L$ (age, comorbidities) blocks all
  ...
  \textcolor{accent}{\textbf{The causal gap:}} distance between what we
  want and what we can estimate --- Step 5 quantifies this gap.
\end{frame}
...
\begin{frame}{Step 6: Define the Statistical Estimand}
  ...
  \textbf{Under identifiability, express the causal parameter as observed data:}

  \textbf{Causal estimand:} $\psi = E[Y^{(1)}] - E[Y^{(0)}]$

  \textbf{Statistical estimand:}
  \[
    \psi = \sum_{l} \left( E[Y \mid A=1, L=l] - E[Y \mid A=0, L=l] \right) P(L=l)
  \]

  \textbf{Interpretation:}
  \begin{itemize}
    \item For each covariate profile $l$, compare expected outcomes,
      weighted by the population distribution of $L$
...
\begin{frame}{Step 7: Choose and Implement an Estimator}
  \small
  \textbf{Select a method to estimate the statistical parameter:}
  \vspace{0.3em}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Propensity score methods (IPW, matching)
  ...
  \textbf{Best practice:} use \textbf{outcome-blind} model selection
  (don't peek at $A \to Y$!); prefer \textbf{doubly-robust} methods
  (TMLE, AIPW).
  \vspace{0.4em}

  \textcolor{accent}{\textbf{Module G}} will cover estimation in depth, with TMLE on this example.
\end{frame}
...
\begin{frame}{A good idea whose time has come}
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.75\linewidth]{art/navigator}
	\end{figure}
...
\begin{frame}{Module D Summary}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Causal parameter} --- formal estimand ($E[Y^{(1)}] - E[Y^{(0)}]$)
