...
\begin{frame}{The Navigator Carries the Study Plan into Estimation}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \vspace{0.7em}
      \textbf{Target: marginal 12-month risk difference}
      \[
...
\begin{frame}{Crude Association Reflects Confounding by Indication}
  \small
  \begin{columns}[c]
    ...
    \begin{column}{0.25\textwidth}
      \begin{block}{Crude RD}
        \centering
        {\large\(\mathbf{+0.76}\)}\\[-0.1em]
        {\scriptsize percentage points}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{G-Computation Standardizes Outcome Predictions}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      For every participant, obtain both intervention predictions:
      \[
        \widehat Q(1,W_i)
        \qquad
        \widehat Q(0,W_i).
...
\begin{frame}{The Treatment Model Estimates Vaccination Probability}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      \vspace{0.55em}
      IPTW uses \(g(W)\) to reweight observed outcomes:
      \[
        \widehat\Psi_{\mathrm{IPTW}}=-2.94\text{ pp}.
      \]
    \end{column}
...
\begin{frame}{Overlap Informs the Positivity Assessment}
  \begin{columns}[c]
    ...
    \begin{column}{0.30\textwidth}
      ...
      \vspace{0.35em}
      Both are fitted-data support diagnostics; neither proves structural
      positivity.
    \end{column}
  \end{columns}
  \slidesource{Figure generated by
    ...
    discussion follows Petersen et al. (2012).}
\end{frame}
...
\begin{frame}{Diagnostics and Reproducibility Accompany the Estimate}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \textbf{Support}
      \begin{itemize}
        \item Parametric-GLM comparator: 0.077 to 0.909
        \item TMLE Super Learner \(g\): 0.092 to 0.894
...
\begin{frame}{The TMLE Estimate Is About a 3.0-Percentage-Point Risk Reduction}
  \small
  \begin{columns}[c]
    \begin{column}{0.45\textwidth}
      \centering
      {\color{accent}\fontsize{30}{35}\selectfont\bfseries \(-3.0\) pp}

      \vspace{0.35em}
      \(\text{95\% CI: }-4.6\text{ to }-1.5\text{ pp}\)
    \end{column}
    \begin{column}{0.50\textwidth}
      \begin{block}{Interpretation}
        If the stated assumptions identify the target,
        vaccination is estimated to produce about 3.0 fewer 12-month pneumonia
        hospitalizations per 100 people in this synthetic cohort.
      \end{block}
...
\begin{frame}{TMLE Carries the Study's Causal Assumptions}
  \small
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
...
\begin{frame}{Estimate Completes the Roadmap}
  \small
  \centering
  \begin{tikzpicture}[
