...
\begin{frame}{Navigator Defines the Statistical Problem}
  \small
  \begin{columns}[T]
...
\begin{frame}{The simcausal DAG Includes Baseline History and Treatment Selection}
  ...
  \begin{tikzpicture}[
    ...
    \begin{pgfonlayer}{background}
      ...
      \draw[-{Latex[length=2.2mm]},black!55] (pp.east) to[out=35,in=145] (Y.west);
...
\begin{frame}{Generator Creates Confounding by Indication}
  \footnotesize
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \[
      \begin{aligned}
        &\operatorname{logit}P(Y=1\mid A,W)\\[-0.15em]
        &\quad = -3.00 -0.80A\\
        &\quad +0.25\frac{\mathrm{age}-65}{10}\\
        &\quad +1.60\,\mathrm{priorPneumonia}\\
        &\quad -0.15\,\mathrm{priorVaccine}.
      \end{aligned}
      \]
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \begin{itemize}
        ...
        \item Frozen CSV verified row-for-row against the archived generator
      \end{itemize}
    \end{column}
...
\begin{frame}{TMLE Uses Two Nuisance Fits and One Targeting Step}
  \small
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
    ...
    \begin{pgfonlayer}{background}
      \draw[-{Latex[length=2.2mm]},black!45] (q.north) -- ++(0,0.42cm) -| (t.north);
...
\begin{frame}{Initial Outcome Regression Learns \(Q(A,W)\)}
  \small
  \[
    Q_0(a,w)=E(Y\mid A=a,W=w).
  \]
...
\begin{frame}{Observed Data Show Shared Treatment Support}
  \begin{columns}[c]
    ...
    \begin{column}{0.30\textwidth}
      \small
      \begin{block}{Parametric-GLM comparator}
        Range: \(0.077\) to \(0.909\)\par
        Outside \([0.05,0.95]\): \(0\%\)
      \end{block}

      \textbf{TMLE Super Learner \(g\):} 0.092 to 0.894.\par
      \vspace{0.35em}
      Both are fitted-data support diagnostics; neither tests exchangeability
      or proves structural positivity.
    \end{column}
  \end{columns}
  \slidesource{Figure generated by
    \sourcefile{pneumonia_tmle_technical.R}; Super Learner range from
    \sourcefile{outputs/pneumonia_tmle_technical_diagnostics.csv}; positivity
    guidance follows Petersen et al. (2012).}
\end{frame}

\begin{frame}{Clever Covariate Focuses the Update}
  \small
...
\begin{frame}{Efficient Influence Function Supplies Uncertainty}
  \footnotesize
...
\begin{frame}[fragile]{Manual Step 2: Target, Average, and Infer}
  \begin{lstlisting}[language=R]
fluctuation <- glm(
...
\begin{frame}{Technical Results Share One Risk-Difference Scale}
  \small
  \centering
  \begin{tabular}{@{}lrrr@{}}
    \toprule
...
\begin{frame}{Estimator Comparison Separates Association from Adjustment}
  \centering
  \vspace{-0.2cm}
...
\begin{frame}{Audit Trail Connects Data, Code, Output, and Session State}
  \scriptsize
...
\begin{frame}{TMLE, AIPW, and DML Use Orthogonal Scores}
  \scriptsize
  \begin{tabular}{@{}
...
\begin{frame}{\hspace{0.12em}Technical Takeaways}
  \small
  \begin{enumerate}
...
\begin{frame}{Causal Inference References}
  \footnotesize
  \color{slate}
  \begin{itemize}
    \setlength{\itemsep}{0.55em}
    \item Hernan MA, Robins JM. \emph{Causal Inference: What If}.
      Chapman \& Hall/CRC; 2020.
    \item Petersen ML, Porter KE, Gruber S, Wang Y, van der Laan MJ.
      Diagnosing and responding to violations in the positivity assumption.
      \emph{Statistical Methods in Medical Research}. 2012.
    \item Petersen ML, van der Laan MJ. Causal models and learning from data:
      integrating causal modeling and statistical estimation.
      \emph{Epidemiology}. 2014.
    \item van der Laan MJ, Rose S. \emph{Targeted Learning}. Springer; 2011.
  \end{itemize}
  \slidesource{Full bibliographic entries shown on slide.}
\end{frame}

\begin{frame}{Machine Learning References}
  \small
  \color{slate}
  \begin{itemize}
    \setlength{\itemsep}{0.75em}
    \item Chernozhukov V, et al. Double/debiased machine learning for treatment
      and structural parameters. \emph{The Econometrics Journal}. 2018.
    \item Kennedy EH. Semiparametric doubly robust targeted double machine
      learning: a review. \emph{arXiv:2203.06469}. 2023.
    \item van der Laan MJ, Polley EC, Hubbard AE. Super Learner.
