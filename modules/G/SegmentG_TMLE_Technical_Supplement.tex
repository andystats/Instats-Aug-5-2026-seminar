  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.50\textwidth}
      \begin{block}{Technical version}
        \begin{itemize}
          \item Data generator and cohort verification
...
\begin{frame}{The Generator Creates Confounding by Indication}
  ...
  \slidesource{Coefficients from the archived generator
    \sourcefile{simulate_pneumonia_data.R}.}
\end{frame}

\begin{frame}{The Teaching Cohort Is Fixed and Verifiable}
  \small
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \textbf{One frozen synthetic cohort}
      \begin{itemize}
        \item 5,000 adults
        \item 1,627 vaccinated (32.5\%)
        \item 334 hospitalizations (6.7\%)
        \item Shared unchanged by Modules F and G
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \textbf{Why freeze the data?}
      \begin{itemize}
        \item Everyone analyzes the same rows.
        \item Results on these slides match what you will reproduce.
        \item The archived generator script rebuilds the cohort and
          verifies it row by row.
      \end{itemize}
    \end{column}
  \end{columns}

  \vspace{0.45cm}
  Integrity details --- the file checksum and the generator's
  verification steps --- are documented with the repository materials.
  \slidesource{Generator \sourcefile{simulate_pneumonia_data.R}; frozen
    file \sourcefile{data/pneumonia_data.csv}.}
\end{frame}
...
\begin{frame}{TMLE Uses Two Nuisance Functions and One Targeting Step}
  ...
  \begin{tikzpicture}[
    ...
    % Route the outcome-regression arrow over the treatment-mechanism
    % box: both nuisance models feed the targeting step in parallel.
...
\begin{frame}{The Audit Trail Connects Data, Code, Output, and Session State}
  ...
  \begin{tabular}{@{}p{0.32\textwidth}p{0.58\textwidth}@{}}
    ...
    \sourcefile{simulate_pneumonia_data.R} &
      Structural equations, cohort regeneration, and row-by-row
      verification \\
